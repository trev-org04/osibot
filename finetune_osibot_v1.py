...
class GPTLanguageModel(nn.Module):
    ...
    def generate(self, index, max_new_tokens):
        for _ in range(max_new_tokens):
            ...
            logits, _ = self.forward(index_condition)
            logits = logits[:,-1,:]
...
with open("gpt-v2.pkl", "rb") as file:
...
def get_random_chunk(split):
    filename = "fine-tuning_training.txt" if split == "train" else "fine-tuning_validation.txt"
...
with open("fine-tuned-gpt-v4.pkl", "wb") as file:
    pickle.dump(model, file)
print("Saved fine-tuned model to 'fine-tuned-gpt-v4.pkl'")
